...
void RenderContext_DX11::onBeginRender() {
	...
	if (!_renderTargetView) {
		_createRenderTarget(); //_swapChain , backbuffer , depth stencil
	}
...
// testing
	...
	UINT vertexCount = sizeof(vertexData) / sizeof(vertexData[0]); // vertexData[] no direct function to get the no. of elements in array
	if (!_testVertexBuffer) {
		...
		hr = ctx->Map(_testVertexBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &ms); // map to cpu v ram to upload to gpu
		Util::throwIfError(hr);

		memcpy(ms.pData, vertexData, bd.ByteWidth); 

		//void* __cdecl memcpy(
		//	_Out_writes_bytes_all_(_Size) void* _Dst,  <--D3D11_MAPPED_SUBRESOURCE (void ptr can accept any data type
		//	_In_reads_bytes_(_Size)       void const* _Src, <-- vertexData
		//	_In_                          size_t      _Size  sizeof(Vertex) * vertexCount;
		//);
	...
	if (!_testVertexShader) {
		...
		hr = D3DCompileFromFile(shaderFile, 0, 0, "vs_main", "vs_4_0", 0, 0, bytecode.ptrForInit(), errorMsg.ptrForInit()); //byte code can save in file no need to recompile , in shipping to Xbox cant use D3DCompileFromFile as the related header cant include 
		Util::throwIfError(hr);

		hr = dev->CreateVertexShader(bytecode->GetBufferPointer(), bytecode->GetBufferSize(), nullptr, _testVertexShader.ptrForInit()); //create first time can reuse
		Util::throwIfError(hr);

		D3D11_INPUT_ELEMENT_DESC inputDesc[] =
		{
			{"POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0,  0, D3D11_INPUT_PER_VERTEX_DATA, 0}, //total 12 byte for 3 float in 32bit 
			{"COLOR",    0, DXGI_FORMAT_R8G8B8A8_UNORM,  0, 12, D3D11_INPUT_PER_VERTEX_DATA, 0}, //byte offset start by 12
		};
	...
	if (!_testPixelShader) {
		...
		dev->CreatePixelShader(bytecode->GetBufferPointer(), bytecode->GetBufferSize(), nullptr, _testPixelShader.ptrForInit());//create first time can reuse
		Util::throwIfError(hr);
	}
...
// draw
	...
	UINT stride = sizeof(Vertex);// number of byte per vertex
	UINT offset = 0;
...
void RenderContext_DX11::onSwapBuffers() {
	auto hr = _swapChain->Present(_renderer->vsync() ? 1 : 0, 0); //to ask monitor to swap , in open gl call swap
	Util::throwIfError(hr);
}
