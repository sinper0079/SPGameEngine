...
Renderer_DX11::Renderer_DX11(CreateDesc& desc) {
...
#ifdef _DEBUG
	createDeviceFlags |= D3D11_CREATE_DEVICE_DEBUG; // flag to tell need debug info	
#endif
	...
	hr = d3dDevice->QueryInterface(IID_PPV_ARGS(_d3dDevice.ptrForInit())); //QueryInterface for unique id __uuidof
	Util::throwIfError(hr);
	...
	{
		D3D11_FEATURE_DATA_DOUBLES s = {}; // declares a variable s of type D3D11_FEATURE_DATA_DOUBLES and initializes all its members to zero.
	...
	if (createDeviceFlags & D3D11_CREATE_DEVICE_DEBUG) {
		hr = _d3dDevice->QueryInterface(_d3dDebug.ptrForInit()); // can insert some callback if debug flag activiated	
		Util::throwIfError(hr);
	}
