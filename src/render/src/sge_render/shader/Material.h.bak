...
public: 
	...
	virtual ~Material() = default; 		// to make sure is no arg destructor, if not set default wht happend? 
...
protected:
	...
	template <class V> void _setParam(StrView name, const V& v) // why dont need inline in  header file 
	{
	...
	using Stage = MaterialPass_Stage;
	using VertexStage = MaterialPass_VertexStage;
	using PixelStage = MaterialPass_PixelStage; // to avoid change name problem

	SPtr<Shader> _shader;
	Vector_<SPtr<Pass>, 1>	_passes; // why size=1 
	virtual void onSetShader() = 0; // for asbtract layer .      virtual void onSetShader() {} why this not use pure virtual function ?
